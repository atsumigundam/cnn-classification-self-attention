...
class Attn(nn.Module):
    ...
    def forward(self, textCNN_outputs):
        b_size = textCNN_outputs.size(0)
        att_size_vector = F.relu(self.linear1(textCNN_outputs))
        hidden_size_vector = self.linear2(att_size_vector)
        return F.softmax(hidden_size_vector, dim=1)
class AttnClassifier(nn.Module):
    def __init__(self, hidden_size, out_size):
        super(AttnClassifier, self).__init__()
        self.attn = Attn(hidden_size,24)
        self.main = nn.Linear(hidden_size, out_size)
    def forward(self, textCNN_outputs): # (batch,len(filter_height_list)*kernel_dim_size)
        attns = self.attn(textCNN_outputs)
        feats = textCNN_outputs * attns
...
def train(train_data, word_to_id, id_to_word, model_path):
    ...
    classifier = AttnClassifier(kernel_dim_size*len(filter_height_list), out_size)
    ...
    for epoch in range(epoch_num):
        ...
        for count,batch_data in enumerate(batch_training_data):
            ...
            attn_output, attn= classifier(CNN_out)
            loss = F.cross_entropy(attn_output, y)
            loss.backward()
            CNN_optimizer.step()
